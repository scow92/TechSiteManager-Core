...
/** @typedef {import('../../../server/types/browser-models').BomResult} BomResult */
...
/** @param {WorkPackage} pack @param {User} user @param {ConsumableCatalogueRecord[]} catalogue @param {() => Promise<void>} rerender */
function consumablesView(pack, user, catalogue, rerender) {
  const editable = user.role !== 'viewer' && pack.status !== 'complete';
  const rows = pack.consumableRequirements.map((requirement) => {
    const selector = el('select', { name: 'cataloguePublicId', disabled: editable ? null : '', 'aria-label': 'Catalogue item' }, el('option', { value: '' }, 'Manual requirement'), ...catalogue.filter((item) => item.active || item.publicId === requirement.cataloguePublicId).map((item) => el('option', { value: item.publicId, selected: item.publicId === requirement.cataloguePublicId ? '' : null }, `${item.catalogueReference} — ${item.description}`)));
    const row = el('tr', {}, el('td', {}, selector), ...[['description', requirement.description, 'text'], ['quantityRequired', requirement.quantityRequired, 'number'], ['unit', requirement.unit || '', 'text']].map(([name, value, type]) => el('td', {}, el('input', { name, value, type, min: name === 'quantityRequired' ? Number.EPSILON : null, step: name === 'quantityRequired' ? 'any' : null, disabled: editable ? null : '', 'aria-label': name }))));
    const changed = async () => { await mutatePackage(() => { const cataloguePublicId = /** @type {HTMLSelectElement} */ (selector).value || null; const selected = catalogue.find((item) => item.publicId === cataloguePublicId); requirement.cataloguePublicId = cataloguePublicId; if (selected) { requirement.description = selected.description; requirement.unit = selected.unit; /** @type {HTMLInputElement} */ (row.querySelector('[name="description"]')).value = selected.description; /** @type {HTMLInputElement} */ (row.querySelector('[name="unit"]')).value = selected.unit; } else { requirement.description = /** @type {HTMLInputElement} */ (row.querySelector('[name="description"]')).value; requirement.unit = /** @type {HTMLInputElement} */ (row.querySelector('[name="unit"]')).value; } requirement.quantityRequired = Number(/** @type {HTMLInputElement} */ (row.querySelector('[name="quantityRequired"]')).value); }); };
    for (const control of row.querySelectorAll('input, select')) control.addEventListener(control instanceof HTMLSelectElement ? 'change' : 'input', changed);
  ...
  const clear = editable && pack.consumableRequirements.length ? el('button', { type: 'button', class: 'danger', onclick: async () => { if (confirm('Clear every requirement from this package?')) { await mutatePackage(() => pack.consumableRequirements.splice(0)); await rerender(); } } }, 'Clear package quantities') : null;
  return el('div', { class: 'stack' }, el('p', { class: 'muted' }, 'Package quantities are stored here; shared catalogue definitions are administered separately in Settings.'), ...(add ? [el('div', { class: 'form-actions' }, add, ...(clear ? [clear] : []))] : []), pack.consumableRequirements.length ? el('div', { class: 'table-wrap' }, el('table', {}, el('thead', {}, el('tr', {}, ...['Catalogue item', 'Description', 'Quantity', 'Unit', ...(editable ? [''] : [])].map((heading) => el('th', {}, heading)))), el('tbody', {}, ...rows))) : el('div', { class: 'empty-state' }, el('h2', {}, 'No package requirements recorded'), el('p', {}, 'Add quantities manually or select reusable catalogue definitions.')));
}

/** @param {BomResult} bom */
function bomView(bom) {
  const matchRows = bom.matches.map((match) => el('tr', {}, el('td', {}, match.segmentReference), el('td', {}, match.sku.sku), el('td', {}, match.matchType), el('td', {}, `${match.requiredLengthMetres} → ${match.sku.lengthMetres} m`), el('td', {}, String(match.quantity)), el('td', {}, match.lineTotal.toFixed(2))));
  const unmatched = bom.unmatched.map((entry) => el('li', {}, el('strong', {}, entry.segmentReference), ` — ${entry.message} (${entry.reason})`));
  return el('div', { class: 'stack' }, el('section', { class: 'panel stack' }, el('div', { class: 'section-head' }, el('h2', {}, 'Bill of materials'), el('span', { class: 'count-badge' }, `${bom.matches.length} matched`)), el('p', { class: 'muted' }, 'Simplex schedule rows require two individual leads; duplex rows require one. Next-up matching chooses the shortest compatible active SKU.'), matchRows.length ? el('div', { class: 'table-wrap' }, el('table', {}, el('thead', {}, el('tr', {}, ...['Segment', 'SKU', 'Match', 'Length', 'Quantity', 'Line total'].map((heading) => el('th', {}, heading)))), el('tbody', {}, ...matchRows))) : el('p', { class: 'empty-inline' }, 'No fibre rows matched the active SKU catalogue.'), ...(unmatched.length ? [el('div', { class: 'notice notice-warn' }, el('h3', {}, 'Unmatched fibre rows'), el('ul', {}, ...unmatched))] : []), el('dl', { class: 'settings-list totals' }, el('div', {}, el('dt', {}, 'Fibre total'), el('dd', {}, bom.totals.fibre.toFixed(2))), el('div', {}, el('dt', {}, 'Consumables total'), el('dd', {}, bom.totals.consumables.toFixed(2))), el('div', {}, el('dt', {}, 'Combined estimated total'), el('dd', {}, bom.totals.combined.toFixed(2)))), el('a', { class: 'button secondary', href: `/api/work-packages/${encodeURIComponent(bom.workPackage.publicId)}/bom.csv` }, 'Export BOM spreadsheet')));
}
...
export async function packageView(publicId, user, section = 'details') {
  ...
  const [exporters, presentation, cableReferences, catalogue, bom] = await Promise.all([/** @type {Promise<ExporterDescriptor[]>} */ (api('/plugin-exporters')), /** @type {Promise<PresentationProfile | null>} */ (api('/presentation-profiles/work-package')), /** @type {Promise<import('../../../server/types/browser-models').CableReferenceData>} */ (api(`/sites/${encodeURIComponent(pack.site.publicId)}/cable-reference-data`)), /** @type {Promise<ConsumableCatalogueRecord[]>} */ (api('/catalogue/consumables')), section === 'bom' ? /** @type {Promise<BomResult>} */ (api(`/work-packages/${encodeURIComponent(pack.publicId)}/bom`)) : Promise.resolve(null)]);
  ...
  const content = section === 'work-items' ? workItemsView(pack, user, rerender) : scheduleMedia ? cableScheduleView(pack, user, scheduleMedia, cableReferences, rerender) : section === 'consumables' ? consumablesView(pack, user, catalogue, rerender) : section === 'bom' && bom ? bomView(bom) : section === 'handover' ? handoverView(pack, user, rerender) : selectedView ? renderPresentation(/** @type {PresentationProfile} */ (presentation), selectedView, pack, user, async () => { await refreshPhotos(pack); await rerender(); }) : detailsView(pack, user, rerender);
