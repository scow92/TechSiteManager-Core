...
/** @param {Site} site @param {SiteRecord} point @param {'POST'|'PUT'|'DELETE'} method @param {TerminationPosition | null} position @param {Record<string, unknown>} body */
async function mutatePosition(site, point, method, position, body) {
  const collection = `/sites/${enc(site.publicId)}/termination-points/${enc(point.publicId)}/positions`;
  const temporaryId = method === 'POST' ? `urn:offline:${crypto.randomUUID()}` : null;
  const targetId = position?.publicId || temporaryId;
  const path = method === 'POST' ? collection : `${collection}/${enc(String(targetId))}${method === 'DELETE' ? `?baseVersion=${position?.version}` : ''}`;
  const dependencies = await dependencyMetadata({ sitePublicId: site.publicId, pointPublicId: point.publicId, ...(method === 'POST' ? {} : { targetId }) });
  const result = /** @type {{queued?:boolean} | null} */ (await api(path, {
    method, body: method === 'DELETE' ? undefined : body, queueable: true,
    queueMetadata: { ...dependencies, temporaryId, operationKey: `infrastructure:termination-position:${targetId}:${method}`, entityType: 'termination-points', entityPublicId: targetId, label: `Tray ${body.tray || position?.tray || ''} position ${body.position || position?.position || ''}` }
  }));
  if (result?.queued) {
    await updateCache(collection, (records) => method === 'POST'
      ? [...records, /** @type {SiteRecord} */ ({ publicId: temporaryId, version: 0, ...body })]
  ...
  notify(result?.queued ? 'Position change queued for sync' : 'Position change saved');
}
...
async function terminationSection(site, rooms, points, user, rerender) {
  ...
  const cards = points.map((point) => {
    ...
    const positionRows = owned.map((position) => user.role === 'viewer' ? el('li', {}, `Tray ${position.tray}, position ${position.position} — ${position.label || 'Unlabelled'}`) : handle(el('form', { class: 'inline-record' }, input('Tray', 'tray', String(position.tray), { type: 'number', min: 1, max: point.trayCount || 1, required: '' }), input('Position', 'position', String(position.position), { type: 'number', min: 1, max: point.positionsPerTray || 12, required: '' }), input('Label', 'label', position.label, { maxlength: 120 }), el('button', { type: 'submit' }, 'Save'), el('button', { type: 'button', class: 'danger', onclick: async () => { await mutatePosition(site, point, 'DELETE', position, {}); await rerender(); } }, 'Delete')), async (body) => { await mutatePosition(site, point, 'PUT', position, { tray: Number(body.tray), position: Number(body.position), label: String(body.label), _baseVersion: position.version }); await rerender(); }));
    const addPosition = user.role === 'viewer' ? null : handle(el('form', { class: 'inline-record' }, input('Tray', 'tray', '1', { type: 'number', min: 1, max: point.trayCount || 1, required: '' }), input('Position', 'position', '1', { type: 'number', min: 1, max: point.positionsPerTray || 12, required: '' }), input('Label', 'label', '', { maxlength: 120 }), el('button', { type: 'submit' }, 'Add position')), async (body) => { await mutatePosition(site, point, 'POST', null, { tray: Number(body.tray), position: Number(body.position), label: String(body.label) }); await rerender(); });
