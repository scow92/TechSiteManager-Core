...
| O1 | `public/js/api.js`; `public/js/idb.js`; `public/js/offline.js`; `public/js/offline-ui.js`; `public/js/work-package-store.js` — cached reads, durable/coalesced package drafts including typed schedules and rack corrections, operation/dead-letter stores, FIFO replay, ID remaps and pending logout. |
...
| T11 | `server/test/recovery.test.js` — fresh migration integrity/constraints/down-up, pre-Phase-3/4 preservation and backup/restore of typed cable schedules, assignments, completion, handover bytes, save receipts, infrastructure and provenance. |
...
| W28 | Durable dirty-package and infrastructure queues, FIFO replay, ID remap, scoped conflicts, dead letters and no loss | G17 | **partial** — U3/U4/U8 connect sites, infrastructure, packages and cable schedules, but later workflows remain online-only | **partial** — A1 supplies optimistic conflicts and completed-package rejection, while a product-wide offline reconciliation contract remains incomplete | **partial** — D1/D4/D5 provide versions/audit/receipts; durable client state lives in O1 | **partial** — O1 coalesces and replays package graphs including typed schedules, retains locks/conflicts and remaps infrastructure IDs, but not every future mutation is connected | **partial** — T6/T8/T13/T14/T15 cover replay mechanics through cable schedules; no product-wide no-loss matrix exists | **partial** — package and cable queues are complete, while termination positions and later material/user workflows still need offline policy and evidence. |
...
| W31 | Admin/manager/engineer/viewer permissions, concurrency, locks, audit and safe errors | G2, G8, G10, G16 | **partial** — site, package and cable workflows expose viewer, writer, admin completion and conflict states, but account/workload and later workflow roles remain | **partial** — A1/A2/A5 enforce write/admin boundaries, optimistic versions, origin checks, safe errors and completion locks through schedules; later workflows remain unaccepted | **partial** — D1/D4/D5 provide roles, versions, audit and lock triggers, not every future workflow invariant | **partial** — O1 preserves scoped package/site/cable conflicts and completed-package dead letters, while product-wide reconciliation remains incomplete | **partial** — T2/T4/T5/T13/T14/T15 cover the implemented workflow role/lock matrix, not every product workflow | **partial** — site, package and cable safeguards are verified; users, materials and operations remain. |
...
| W34 | Desktop, tablet, phone, light, dark and print rendering for every applicable workflow | G20 | **partial** — U1/U8 and shared CSS implement responsive, theme and readable print rules through cable schedules | **partial** — APIs are viewport-neutral, but missing later workflow states cannot be rendered | **partial** — theme is local; same-user viewport state is not stored | **partial** — O2 caches the complete current shell but not every future workflow state | **partial** — T12/T15 cover the cable viewport/theme/input matrix plus completed, offline and print states; later workflows remain | **partial** — Phase 4 rendering is evidenced, while product-wide responsive acceptance remains incomplete. |

## Acceptance-flow roll-up

Six critical acceptance flows now have verified implementation evidence at this
...
| `FLOW-02` Site, room, rack, ODF and device editing | W02, W03, W06, W07 | **partial** | Full offline replay for individual termination positions remains. |
...
| `FLOW-12` Offline durability, replay, conflicts, logout and notifications | W27, W28 | **partial** | Termination-position and later workflow replay, resumption and notifications remain. |
...
The 34 detailed workflows contain **1 missing**, **12 partial**, and **21
